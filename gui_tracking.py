    ...
    if os.path.exists(path):
        ...
        while True:
            ...
            for i in range(rclasses.shape[0]):
                ...
                filtered.append([top,left,bottom,right,width,height,labels[rclasses[i]]])
            
            for top,left,bottom,right,width,height,label in filtered:
                #if (right-left) * 2 > w:
                #    continue
                # extract roi                
                ...
                drawMark(frame,(left,top,width,height),label)
                # license recognition
